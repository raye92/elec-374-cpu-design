`timescale 1ns/10ps

module ror_test_tb;

//INPUTS TO DP
reg clock, clear, read;
reg R0in, R1in, R2in, R3in, R4in, R5in, R6in, R7in, R8in, R9in, R10in, R11in, R12in, R13in, R14in, R15in;
reg HIin, LOin, RZin, PCin, MDRin, ALUin, IncPC;
reg R0out, R1out, R2out, R3out, R4out, R5out, R6out, R7out, R8out, R9out, R10out, R11out, R12out, R13out, R14out, R15out;
reg HIout, LOout, ZHIout, ZLOout, PCout, MDRout;
reg MARin, IRin, RYin;
reg [31:0] Mdatain;

parameter Default = 4'b0000, Reg_load1a = 4'b0001, Reg_load1b = 4'b0010, Reg_load2a = 4'b0011, Reg_load2b = 4'b0100, Reg_load3a = 4'b0101, Reg_load3b = 4'b0110, T0 = 4'b0111, T1 = 4'b1000, T2 = 4'b1001, T3 = 4'b1010, T4 = 4'b1011, T5 = 4'b1100, T6 = 4'b1101;
reg [3:0] Present_state = Default;

DataPath DP(
    clock, clear, read, 
    R0in, R1in, R2in, R3in, R4in, R5in, R6in, R7in, R8in, R9in, R10in, R11in, R12in, R13in, R14in, R15in,
    HIin, LOin, RZin, PCin, MDRin, ALUin, IncPC,
    R0out, R1out, R2out, R3out, R4out, R5out, R6out, R7out, R8out, R9out, R10out, R11out, R12out, R13out, R14out, R15out,
    HIout, LOout, ZHIout, ZLOout, PCout, MDRout,
    MARin, IRin, RYin, Mdatain
);

initial begin
    clock = 0;
    forever #10 clock = ~clock;
end

always @(posedge clock) begin
    case (Present_state)
        Default   : Present_state = Reg_load1a;
        Reg_load1a: Present_state = Reg_load1b;
        Reg_load1b: Present_state = Reg_load2a;
        Reg_load2a: Present_state = Reg_load2b;
        Reg_load2b: Present_state = Reg_load3a;
        Reg_load3a: Present_state = Reg_load3b;
        Reg_load3b: Present_state = T0;
        T0        : Present_state = T1;
        T1        : Present_state = T2;
        T2        : Present_state = T3;
        T3        : Present_state = T4;
        T4        : Present_state = T5;
        T5        : Present_state = T6;
    endcase
end

always @(Present_state) begin
    case(Present_state)
        Default: begin
            read <= 0; clear <= 1;
            R0in <= 0; R1in <= 0; R2in <= 0; R3in <= 0; R4in <= 0; R5in <= 0; R6in <= 0; R7in <= 0; R8in <= 0; R9in <= 0; R10in <= 0; R11in <= 0; R12in <= 0; R13in <= 0; R14in <= 0; R15in <= 0;
            HIin <= 0; LOin <= 0; RZin <= 0; PCin <= 0; MDRin <= 0; ALUin <= 0; IncPC <= 0;
            R0out <= 0; R1out <= 0; R2out <= 0; R3out <= 0; R4out <= 0; R5out <= 0; R6out <= 0; R7out <= 0; R8out <= 0; R9out <= 0; R10out <= 0; R11out <= 0; R12out <= 0; R13out <= 0; R14out <= 0; R15out <= 0;
            HIout <= 0; LOout <= 0; ZHIout <= 0; ZLOout <=0; PCout <= 0; MDRout <= 0;
            MARin <= 0; IRin <= 0; RYin <= 0; Mdatain <= 32'h00000000;
            #10 clear <= 0;
        end
        Reg_load1a: begin
            #1 Mdatain <= 32'h00000000; // Load R7 (Dest initially 0)
            read <= 1; MDRin <= 1;
            #15 read <= 0; MDRin <= 0;
        end
        Reg_load1b: begin
            #1 MDRout <= 1; R7in <= 1;
            #15 MDRout <= 0; R7in <= 0;
        end
        Reg_load2a: begin
            #1 Mdatain <= 32'h12345678; // Load R0 (Value to rotate)
            read <= 1; MDRin <= 1;
            #15 read <= 0; MDRin <= 0;
        end
        Reg_load2b: begin
            #1 MDRout <= 1; R0in <= 1;
            #15 MDRout <= 0; R0in <= 0;
        end
        Reg_load3a: begin
            #1 Mdatain <= 32'h00000004; // Load R4 (Rotation count)
            read <= 1; MDRin <= 1;
            #15 read <= 0; MDRin <= 0;
        end
        Reg_load3b: begin
            #1 MDRout <= 1; R4in <= 1;
            #15 MDRout <= 0; R4in <= 0;
        end
        T0: begin
            PCout <= 1; MARin <= 1; IncPC <= 1; 
            #15 PCout <= 0; MARin <= 0; IncPC <= 0;
        end
        T1: begin
            read <= 1; MDRin <= 1;
            Mdatain <= 32'h3B820000; // opcode for "ror R7, R0, R4"
            #15 read <= 0; MDRin <= 0; Mdatain <= 32'h0;
        end
        T2: begin
            MDRout <= 1; IRin <= 1; ALUin <= 1;
            #15 MDRout <= 0; IRin <= 0; ALUin <= 0;
        end
        T3: begin
            #1 R0out <= 1; RYin <= 1; // [cite: 28]
            #15 R0out <= 0; RYin <= 0;
        end
        T4: begin
            #1 R4out <= 1; RZin <= 1; // Revised for count in R4 
            #14 R4out <= 0; RZin <= 0;
        end
        T5: begin
            #1 ZLOout <= 1; R7in <= 1; // [cite: 28]
            #15 ZLOout <= 0; R7in <= 0;
        end
    endcase
end
endmodule